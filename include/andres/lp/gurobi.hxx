...
inline void
Gurobi::setNumberOfThreads(const size_t numberOfThreads)
{
    gurobiEnvironment_.set(GRB_IntParam_Threads, numberOfThreads);
}

inline void
Gurobi::setAbsoluteGap(const double gap)
{
    gurobiEnvironment_.set(GRB_DoubleParam_MIPGapAbs, gap);
}

inline void
Gurobi::setRelativeGap(const double gap)
{
    gurobiEnvironment_.set(GRB_DoubleParam_MIPGap, gap);
}

inline void
Gurobi::setVerbosity(const bool verbosity)
{
    if (verbosity) {
...
inline void
Gurobi::setPreSolver(const PreSolver preSolver, const int passes)
{
    switch (preSolver) {
...
inline void
Gurobi::setLPSolver(const LPSolver lpSolver)
{
    switch (lpSolver) {
        ...
        case LP_SOLVER_SIFTING:
            ...
            gurobiEnvironment_.set(GRB_IntParam_SiftMethod,
                                   1); // moderate, 2 = aggressive
            break;
    }
}

inline void
Gurobi::initModel(const size_t numberOfVariables, const double* coefficients)
{
    if (gurobiModel_ != nullptr)
        delete gurobiModel_;
    ...
    gurobiVariables_ = gurobiModel_->addVars(nullptr, ub.data(), nullptr,
                                             nullptr, nullptr, nVariables_);
...
inline void
Gurobi::addVariables(const size_t numberOfVariables, const double* coefficients)
{
    initModel(numberOfVariables, coefficients);
}
...
Gurobi::optimize()
{
    ...
    // size_t fractional = 0;
    // for (size_t idx = 0; idx < nVariables_; ++idx)
    //     if (variableValue(idx) > EPS && variableValue(idx) < 1 - EPS)
    //         ++fractional;
    // std::cout << fractional << "/" << nVariables_ << std::endl;
}

inline double
Gurobi::variableValue(const size_t variableIndex) const
{
...
template <class VariableIndexIterator, class CoefficientIterator>
void
Gurobi::addConstraint(VariableIndexIterator viBegin,
                      VariableIndexIterator viEnd,
                      CoefficientIterator coefficient, const double lowerBound,
                      const double upperBound)
{
    GRBLinExpr expression;
    for (; viBegin != viEnd; ++viBegin, ++coefficient) {
        expression +=
            (*coefficient) * gurobiVariables_[static_cast<size_t>(*viBegin)];
    }
...
template <class Iterator>
void
Gurobi::setStart(Iterator valueIterator)
{
    for (size_t j = 0; j < nVariables_; ++j, ++valueIterator)
        gurobiVariables_[j].set(GRB_DoubleAttr_Start,
                                static_cast<double>(*valueIterator));

    gurobiModel_->update();
}

inline double
Gurobi::objective() const
{
    return gurobiModel_->get(GRB_DoubleAttr_ObjVal);
}

inline double
Gurobi::label(const size_t variableIndex) const
{
    return gurobiVariables_[variableIndex].get(GRB_DoubleAttr_X);
}
